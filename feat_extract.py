...
def parse_audio_files(parent_dir,file_ext='*.wav'):     # Choose only .wav files from the directory
    ...
    for label, sub_dir in enumerate(sub_dirs):
        if os.path.isdir(os.path.join(parent_dir, sub_dir)):
            for fn in glob.glob(os.path.join(parent_dir, sub_dir, file_ext)):
                ...
                print ('Dim. features: ', features.shape)            # Print the dimension of the features
                print ('files processed: ', features.shape[0]+1)     # Print the number of files processed.

                features = np.vstack([features,ext_features])
                labels = np.append(labels, label)
            print("Done extracting %s " % (sub_dir))                 # Print when finished extracting features from a folder.
    #np.savetxt('features.csv', features, delimiter=',')             # For user use only keep data in a csv, to be easily reviewed..
